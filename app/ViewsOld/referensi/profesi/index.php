        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="profesiTable" class="table table-sm table-hover table-bordered">
                        <thead>
                            <tr>
...
$(document).ready(function() {
    table = $('#profesiTable').DataTable({
        ...
        processing: true,
        scrollCollapse: true,
        ajax: '<?= base_url('referensi/profesi/ajaxList') ?>',
        columns: [{
                data: null,
                class: 'text-center',
                orderable: false,
    ...
    $('#addBtn').on('click', function() {
        $.ajax({
            type: "GET",
            url: "<?= base_url('referensi/profesi/form') ?>",
            dataType: "html",
            success: function(response) {
                ...
                $('#labelModal').text('👨‍🏫 Tambah Profesi');
                $('#id').val('');
                $('#agama').val('');
                $('#status').val('1');
    ...
    $('#profesiTable').on('click', '.editBtn', function() {
        const id = $(this).data('id');
        $.get('<?= base_url('referensi/profesi/get') ?>/' + id, function(data) {
            if (!$('.viewmodal').html()) {
    ...
    $('#profesiTable').on('click', '.deleteBtn', function() {
        const id = $(this).data('id');
        Swal.fire({
            title: "Yakin Hapus?",
            ...
            if (result.isConfirmed) {
                $.get('<?= base_url('referensi/profesi/delete') ?>/' + id, () => {
                    Swal.fire({
                        title: "Terhapus!",
    ...
    $('#modulForm').submit(function(e) {
        ...
        const url = $('#ID').val() ? '<?= base_url('referensi/profesi/update') ?>' :
            '<?= base_url('referensi/profesi/store') ?>';
