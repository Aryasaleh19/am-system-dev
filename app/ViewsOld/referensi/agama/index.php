        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-body">
                    ...
                    <table id="agamaTable" class="table table-sm table-hover table-bordered">
                        <thead>
                            <tr>
...
$(document).ready(function() {
    table = $('#agamaTable').DataTable({
        ...
        processing: true,
        scrollCollapse: true,
        ajax: '<?= base_url('referensi/agama/ajaxList') ?>',
        columns: [{
                data: null,
                class: 'text-center',
                orderable: false,
    ...
    $('#addBtn').on('click', function() {
        $.ajax({
            type: "GET",
            url: "<?= base_url('referensi/agama/form') ?>",
            dataType: "html",
            success: function(response) {
                ...
                $('#labelModal').text('☪️ Tambah Agama');
                $('#id').val('');
                $('#agama').val('');
                $('#status').val('1');
    ...
    $('#agamaTable').on('click', '.editBtn', function() {
        const id = $(this).data('id');
        $.get('<?= base_url('referensi/agama/get') ?>/' + id, function(data) {
            if (!$('.viewmodal').html()) {
    ...
    $('#agamaTable').on('click', '.deleteBtn', function() {
        const id = $(this).data('id');
        Swal.fire({
            title: "Yakin Hapus?",
            ...
            if (result.isConfirmed) {
                $.get('<?= base_url('referensi/agama/delete') ?>/' + id, () => {
                    Swal.fire({
                        title: "Terhapus!",
    ...
    $('#modulForm').submit(function(e) {
        ...
        const url = $('#ID').val() ? '<?= base_url('referensi/agama/update') ?>' :
            '<?= base_url('referensi/agama/store') ?>';
